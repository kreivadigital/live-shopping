...
final class Database
{
    ...
    public static function connection(): PDO
    {
        ...
        try {
            match ($driver) {
                'mysql' => self::migrateMysql(self::$pdo),
                'pgsql' => self::migratePgsql(self::$pdo),
                default => self::migrateSqlite(self::$pdo),
            };
    ...
    /**
     * Creates the PostgreSQL connection (Supabase).
     */
    private static function connectPgsql(): PDO
    {
        if (!extension_loaded('pdo_pgsql')) {
            throw new RuntimeException('La extensión pdo_pgsql no está habilitada en este hosting');
        }

        $host = (string) Config::get('DB_HOST', '127.0.0.1');
        $port = (string) Config::get('DB_PORT', '5432');
        $db = (string) Config::get('DB_NAME', 'postgres');
        $user = (string) Config::get('DB_USER', 'postgres');
        $pass = (string) Config::get('DB_PASS', '');

        // Supabase presenta un certificado con CA propia: con verify-full la conexión
        // falla aunque las credenciales estén bien. Y hay que apuntar al *pooler*, no
        // al host directo: el directo sólo resuelve por IPv6.
    ...
    /**
     * Ensures the PostgreSQL schema is in place. Idempotente: todo va con IF NOT EXISTS,
     * así se puede ejecutar en cada arranque igual que las otras dos.
     */
    private static function migratePgsql(PDO $pdo): void
    {
        ...
        // Transmisión propia. El default \'youtube\' mantiene el comportamiento de
        // siempre en cualquier tienda que no se toque explícitamente.
        $pdo->exec(
            'ALTER TABLE live_sessions
                ADD COLUMN IF NOT EXISTS stream_source varchar(20) NOT NULL DEFAULT \'youtube\',
                ADD COLUMN IF NOT EXISTS stream_uid varchar(64) NULL,
                ADD COLUMN IF NOT EXISTS playback_url text NULL'
        );

        // MySQL refresca updated_at solo; en Postgres hace falta el trigger, y sin él
        // los ORDER BY updated_at del inventario y la consola dejan de ordenar.
        $pdo->exec(
